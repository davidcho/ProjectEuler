...
public class A1 {
	...
	static void prob3() {
		...
		while (!i.equals(BigInteger.ONE)) {
			...
			temp = new BigInteger(prime + "");
			while (i.mod(temp).equals(BigInteger.ZERO))
				i = i.divide(temp);
		}
	...
	static void prob7() {
		 ...
		 */
		long start = System.currentTimeMillis();
		System.out.println("7: " + Prime.nthPrime(10001));
		System.out.println("\tTime taken: "
				+ (System.currentTimeMillis() - start));
	}
			...
			if (array[i].length() > 0)
				if (array[i].length() < 6) {
					...
					for (int j = 0; j < array[i].length(); j++)
						prod *= Primitive.intValue(array[i].charAt(j));
					if (prod > max)
						max = prod;
				} else {
					prod = Primitive.intValue(array[i].charAt(0))
							* Primitive.intValue(array[i].charAt(1))
							* Primitive.intValue(array[i].charAt(2))
							* Primitive.intValue(array[i].charAt(3))
							* Primitive.intValue(array[i].charAt(4));
					if (prod > max)
						max = prod;
					for (int j = 5; j < array[i].length(); j++) {
						prod /= Primitive.intValue(array[i].charAt(j - 5));
						prod *= Primitive.intValue(array[i].charAt(j));
						if (prod > max)
							max = prod;
					}
	...
	static void prob9() {
		...
		/**
		 * for all m,n and m>n: a Pythagorean triplet can be formed with: a =
		 * m^2 - n^2; b = 2mn; c = m^2 + n^2
		 */

		// a + b + c = 1000;
		// 2m^2 + 2mn = 1000
		// m(m+n) = 500
		// m and n must be of opposite parity
		long start = System.currentTimeMillis();

		int n;
		for (int m = 1; m < Math.sqrt(500); m++)
			if (500 % m == 0 && (500 / m % 2 == 1 || m % 2 == 1)) {
				n = 500 / m - m;
				int a = Math.abs(m * m - n * n);
				int b = 2 * m * n;
				int c = m * m + n * n;
				if (Pythagorean.isTriplet(a, b, c) && a + b + c == 1000) {
	...
	static void prob10() {
		...
		// right now this algorithm runs on n^2. Change into sieve - sum all and
		// subtract composites
		long start = System.currentTimeMillis();
		...
		while (prime.compareTo(limit) < 0) {
			sum = sum.add(prime);
			prime = Prime.nextBigPrime(prime);
		}
	...
	static void prob11() {
		...
		for (int i = 0; i < 400; i++)
			numbers[i] = Primitive.intValue(array[i]);
		...
		for (int i = 0; i < 20; i++)
			for (int j = 0; j < 17; j++) {
				prod = numbers[20 * i + j] * numbers[20 * i + j + 1]
						* numbers[20 * i + j + 2] * numbers[20 * i + j + 3];
				if (prod > max)
					max = prod;
			}
		...
		for (int i = 0; i < 17; i++)
			for (int j = 0; j < 20; j++) {
				prod = numbers[20 * i + j] * numbers[20 * i + j + 20]
						* numbers[20 * i + j + 40] * numbers[20 * i + j + 60];
				if (prod > max)
					max = prod;
			}
		...
		for (int i = 0; i < 17; i++)
			for (int j = 0; j < 17; j++) {
				prod = numbers[20 * i + j] * numbers[20 * i + j + 21]
						* numbers[20 * i + j + 42] * numbers[20 * i + j + 63];
				if (prod > max)
					max = prod;
			}
		...
		for (int i = 0; i < 17; i++)
			for (int j = 3; j < 20; j++) {
				prod = numbers[20 * i + j] * numbers[20 * i + j + 19]
						* numbers[20 * i + j + 38] * numbers[20 * i + j + 57];
				if (prod > max)
					max = prod;
			}
	...
	static void prob12() {
		...
		int sum = 0;
		int count = 1;
		while (Primitive.getNumDivisors(sum) < 500)
			sum += count;
	...
	static void prob13() {
		...
		for (int i = 49; i >= 0; i--) {
			for (int j = 0; j < 100; j++)
				sum += Primitive.intValue(s.charAt(50 * j + i));
			digits[i] = sum % 10;
			sum /= 10;
		}
		...
		for (int i = 0; i < 10 - count; i++) {
			b = b.multiply(ten);
			b = b.add(new BigInteger(digits[i] + ""));
		}
	...
	static void prob14() {
		...
		for (int i = 113383; i < 113384; i++) {
			...
			while (temp != 1)
				if (temp % 2 == 0) {
					temp = Function.even(temp);
					count++;
				} else {
					temp = Function.iterate(temp);
					count += 2;
				}
		...
		System.out.println("14: " + returnable);
		System.out.println("\tTime taken: "
				+ (System.currentTimeMillis() - start));
	}
	...
	static void prob15() {
		...
		/**
		 * Starting in the top left corner of a 22 grid, and only being able to
